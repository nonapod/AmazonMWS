...
abstract class MarketplaceWebService_Model
{
    ...
    {
        // fix broken api, datetime was not coming through as object
            ...
            if (is_array($fieldType)) {
                if ($this->isComplexType($fieldType[0])) {
                    ...
                    if ($elements->length >= 1) {
                        // this part of the api was broken; fixed
                        $path = str_replace('_', DIRECTORY_SEPARATOR, $fieldType[0]) . ".php";
                        $path = explode(DIRECTORY_SEPARATOR, $path);
                        unset($path[0]);
                        require_once (dirname(__FILE__).'/'.implode($path, DIRECTORY_SEPARATOR));
                ...
                if ($this->isComplexType($fieldType)) {
                    ...
                    if ($elements->length == 1) {
                        // this part of the api was broken; fixed
                        $path = str_replace('_', DIRECTORY_SEPARATOR, $fieldType) . ".php";
                        $path = explode(DIRECTORY_SEPARATOR, $path);
                        unset($path[0]);
                        require_once (dirname(__FILE__).'/'.implode($path, DIRECTORY_SEPARATOR));
